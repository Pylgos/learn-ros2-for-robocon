...
namespace laserscan_merger {
LaserScanMarger::LaserScanMarger(const rclcpp::NodeOptions &options)
    : LaserScanMarger("laserscan_merger_node", options) {}

LaserScanMarger::LaserScanMarger(const std::string name,
                                 const rclcpp::NodeOptions &options)
    : Node(name, options), scan_timeout_{0, 0} {
...
void LaserScanMarger::timer_callback() {
  ...
  for (const auto &scan : scan_buffer_) {
    ...
    // if (stamp.sec > latest.sec || (stamp.sec == latest.sec && stamp.nanosec >
    // latest.nanosec)){
    //   latest = stamp;
    // }
  }
  ...
  std::fill(out.ranges.begin(), out.ranges.end(),
  ...
  for (const auto &scan : scan_buffer_) {
    ...
    try {
      const auto transform_msg = tf_buf_->lookupTransform(
          laser_frame_, val->header.frame_id, oldest /*val->header.stamp*/);
      ...
      pcl_ros::transformPointCloud(laser_frame_, transform_msg, pc2_msg,
                                   pc2_msg_transformed);
      pcl::fromROSMsg(pc2_msg_transformed, sub_cloud);
      pcl::concatenate(cloud, sub_cloud, cloud);
    } catch (std::exception &e) {
      // RCLCPP_WARN(get_logger(), "Error while processing scan what(): %s",
      // e.what());
    }
  }
  ...
  for (const auto &pt : cloud.points) {
    ...
    if (idx < out.ranges.size()) {
      if (std::isnan(out.ranges[idx]) ||
          (out.ranges[idx] < range && out.range_min <= range &&
           range <= out.range_max)) {
...
RCLCPP_COMPONENTS_REGISTER_NODE(laserscan_merger::LaserScanMarger)
